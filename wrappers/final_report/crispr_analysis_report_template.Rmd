...
for (i in list.files(paste0(snakemake@params$wdir, path), "gene_summary.txt", recursive=T)){
  cat(sprintf("\t\t* [%s](%s/%s)\n", basename(i), path, i))
}
```
    * `*.sgRNAs_summary.txt` (file) - result of DE on sgRNA level:
```{r echo = FALSE, results = "asis"}
path = "DE_results/MAGeCK"
for (i in list.files(paste0(snakemake@params$wdir, path), "sgrna_summary.txt", recursive=T)){
  cat(sprintf("\t\t* [%s](%s/%s)\n", basename(i), path, i))
}
```
    * `*.graphs.pdf` (file) - summary results with plots for TOP `r snakemake@params$top` genes:
