    ...
    for line in duplicates_line_list:
        ...
        if len(set(genes)) == 1:
            for i, gene in enumerate(genes):
                new_lines.append(
                    gene + "\t" + sgRNAs[i] + "\t" + data[2] + "\t" + str(int(counts_number / len(genes))) + "\n")
        else:
            for gene in genes:
                if gene not in gene_dict.keys():
            ...
            if cov_sum != 0:
                if not 0 in gene_coverage.values():
                    ...
                    for i, gene in enumerate(genes):
                        new_lines.append(gene + "\t" + sgRNAs[i] + "\t" + data[2] + "\t" + str(
                            int(counts_number * float(gene_coverage[gene]))) + "\n")
                    ...
                    if cov_sum > counts_number:
                        ...
                        for i, gene in enumerate(non_zero_genes):
                            ...
                            new_lines.append(gene + "\t" + non_zero_sgRNAs[i] + "\t" + data[2] + "\t" + str(
                                int(gene_coverage[gene])) + "\n")
                            if len(gene_dict[gene]) <= 6:
                                gene_dict[gene].append(int(gene_coverage[gene]))
                        for i, gene in enumerate(zero_genes):
                            new_lines.append(gene + "\t" + zero_sgRNAs[i] + "\t" + data[2] + "\t" + str(
                                int(int(counts_number) / len(zero_genes))) + "\n")
