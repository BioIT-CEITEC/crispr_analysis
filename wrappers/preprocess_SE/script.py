######################################
# wrapper for rule: preprocess_PE
######################################
import os
import math
import subprocess
...
command = "mkdir -p $(dirname " + snakemake.output.R1 + ") >> " + log_file + " 2>&1"
f = open(log_file, 'at')
...
command = "(time zcat " + snakemake.output.R1 + " | awk 'NR == 2 || NR % 4 == 2' > " + snakemake.output.seqs + ") 2>> " + log_file
f = open(log_file, 'at')
f.write("## COMMAND: "+command+"\n")
f.close()
shell(command)
...
command = "(time cut -c 1-" + str(snakemake.params.guide_len) + " " + snakemake.output.seqs + " > " + snakemake.output.inserts + ") 2>> " + log_file
f = open(log_file, 'at')
f.write("## COMMAND: "+command+"\n")
f.close()
shell(command)

command = "mkdir -p " + snakemake.params.prefix + " >> " + log_file + " 2>&1"
f = open(log_file, 'at')
f.write("## COMMAND: "+command+"\n")
f.close()
shell(command)
...
command = "(time fastqc -o "+snakemake.params.prefix+" "+extra+" --threads "+str(snakemake.threads)+" "+snakemake.output.R1+") >> "+log_file+" 2>&1"
f = open(log_file, 'at')
f.write("## COMMAND: "+command+"\n")
f.close()
shell(command)

command = ' sed -r "s:<h2>[ ]*Summary[ ]*<\/h2><ul>:&<li><b>Return to <a href=\'../final_report.html\'>start page<\/a><\/b><\/li>:" '+snakemake.params.qc_html_R1_tmp+' > '+snakemake.params.qc_html_R1
f = open(log_file, 'at')
f.write("## COMMAND: "+command+"\n")
f.close()
shell(command)

command = "rm -f "+snakemake.params.qc_html_R1_tmp+" >> "+log_file+" 2>&1"
f = open(log_file, 'at')
f.write("## COMMAND: "+command+"\n")
f.close()
shell(command)

# command = "rm -f "+snakemake.params.qc_html_R1_tmp.replace(".html", ".zip")+" >> "+log_file+" 2>&1"
# f = open(log_file, 'at')
# f.write("## COMMAND: "+command+"\n")
# f.close()
# shell(command)
